...
def _parse_events_ndjson() -> Tuple[pd.DataFrame, int]:
    event_records: List[Dict[str, Any]] = []
    skipped_lines = 0

    with RAW_EVENTS.open("r", encoding="utf-8") as records_infile:
        for line_number, line in enumerate(records_infile, start=1):
            ...
            try:
                ...
                skipped_lines += 1
                print(f"[events.ndjson] skipped corrupted JSON at line {line_number}")
                continue

            if not isinstance(parsed_record, dict):
                skipped_lines += 1
                print(f"[events.ndjson] skipped non-object JSON at line {line_number}")
                continue

            event_records.append(parsed_record)

    return pd.DataFrame(event_records), skipped_lines


def ingest_events(conn) -> BronzeIngestResult:
    events_df, skipped_events = _parse_events_ndjson()
